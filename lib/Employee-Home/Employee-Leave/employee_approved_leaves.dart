...
import 'package:hrm_final_project/Employee-Home/Employee-Leave/employee_pending_leaves.dart';
...
class EmployeeApprovedLeave extends StatefulWidget {
  int? uid;
  EmployeeApprovedLeave({super.key, required this.uid});

  @override
  State<EmployeeApprovedLeave> createState() => _EmployeeApprovedLeaveState();
}

class _EmployeeApprovedLeaveState extends State<EmployeeApprovedLeave> {
  ...
  void initState() {
    super.initState();
    _widget = EmployeeApprovedLeave(uid: widget.uid);
  }
  ...
  Future<List<Leavewithusermodel>> fetchleaveapplication(int id) async {
    ...
    final response = await http.get(Uri.parse(
        'http://$ip/HrmPractise02/api/Leave/NewApprovedLeaveGet?uid=$id')); // is ma aik variable bnaya ha response ka name sa or phir get method ka through api ko hit kar rahay hn is ka data aik data variable ma store karway ga
