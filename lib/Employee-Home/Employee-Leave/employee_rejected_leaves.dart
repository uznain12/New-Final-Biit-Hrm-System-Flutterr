...
class EmployeeRejectedLeave extends StatefulWidget {
  int? uid;
  EmployeeRejectedLeave({super.key, required this.uid});

  @override
  State<EmployeeRejectedLeave> createState() => _EmployeeRejectedLeaveState();
}

class _EmployeeRejectedLeaveState extends State<EmployeeRejectedLeave> {
  ...
  void initState() {
    super.initState();
    _widget = EmployeeRejectedLeave(uid: widget.uid);
  }
  ...
  Future<List<Leavewithusermodel>> fetchleaveapplication(int id) async {
    ...
    final response = await http.get(Uri.parse(
        'http://$ip/HrmPractise02/api/Leave/RejectedLeaveGet?uid=$id')); // is ma aik variable bnaya ha response ka name sa or phir get method ka through api ko hit kar rahay hn is ka data aik data variable ma store karway ga
